  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [users, fishermen, drops, offers, sales, reservations, premiumSubs, notifications] = await Promise.all([
        ...
        supabase.from('payments').select('id', { count: 'exact', head: true }).eq('status', 'active').in('plan', ['premium_monthly', 'premium_annual', 'premium_plus_monthly', 'premium_plus_annual', 'premium', 'fisherman_basic', 'fisherman_pro']),
