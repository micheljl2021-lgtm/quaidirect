-- Créer un trigger qui appelle la fonction edge pour envoyer les notifications
-- quand un nouveau drop est créé

-- Fonction qui déclenche l'edge function
...
-- Créer le trigger sur la table drops
DROP TRIGGER IF EXISTS trigger_notify_drop_followers ON drops;

CREATE TRIGGER trigger_notify_drop_followers
  AFTER INSERT ON drops
  FOR EACH ROW
  EXECUTE FUNCTION notify_drop_followers();