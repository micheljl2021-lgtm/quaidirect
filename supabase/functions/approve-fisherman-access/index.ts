...
serve(async (req) => {
  ...
  try {
    ...
    if (roleError) {
      ...
      return errorResponse("Erreur de vérification des permissions", 500, origin);
    }

    if (!adminRole) {
      console.warn(`User ${user.email} attempted to access admin function without admin role`);
      return errorResponse("Accès administrateur requis", 403, origin);
    }
    ...
    return errorResponse(error.message || "Erreur inconnue", 500, origin);
  }
});
