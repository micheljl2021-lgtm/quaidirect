...
const logStep = (step: string, details?: any) => {
  console.log(`[SEND-SMS-NOTIFICATION] ${step}`, details ? JSON.stringify(details) : '');
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request
    const { drop_id, contact_ids, template_id, custom_message }: SendSmsNotificationRequest = await req.json();
    ...
    const { data: drop, error: dropError } = await supabase
      .from('drops')
      .select('id, species, fisherman_id')
      .eq('id', drop_id)
    ...
    if (dropError || !drop) {
      return new Response(
        JSON.stringify({ error: 'Drop not found or access denied' }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    logStep('Drop loaded', { dropId: drop.id, species: drop.species });

    // Get contacts - either specific ones or all with valid phone numbers
    ...
    if (!messageTemplate) {
      ...
      messageTemplate = defaultTemplate?.body || 
        'Arrivage ! Poisson frais du jour : {{species}} - Voir les détails : {{drop_link}}';
    }

    // Generate drop link
    const dropLink = `${Deno.env.get('SITE_URL') || 'https://quaidirect.fr'}/drop/${drop.id}`;
    
    // Format species list
    const speciesList = Array.isArray(drop.species) 
      ? drop.species.join(', ') 
      : drop.species || 'Poisson frais';
    ...
    for (const contact of contacts) {
      try {
        ...
        const message = messageTemplate
          .replace(/{{drop_link}}/g, dropLink)
          .replace(/{{species}}/g, speciesList)
          .replace(/{{first_name}}/g, contact.first_name || '')
          .replace(/{{last_name}}/g, contact.last_name || '')
          .replace(/{{boat_name}}/g, fisherman.boat_name || '');
